...
func (proto *ARPInstance) Start(out chan<- []byte) {
	...
	log.Info.Println("starting ARP")
	go func() {
		defer func() {
			log.Info.Println("stopping ARP")
		}()
		for arp := range proto.Packets {
			...
			case layers.ARPReply:
				proto.Cache.Set(net.IP(arp.SourceProtAddress), net.HardwareAddr(arp.SourceHwAddress))
			default:
				log.Error.Println("got an ARP packet that is not a request nor a reply")
			}
		}
	}()
}
...
func (proto *ARPInstance) Reply(request *layers.ARP) ([]byte, error) {
	...
	if !ok {
		return []byte{}, errors.New(net.IP(request.DstProtAddress).String(), "not found in local ARP table")
	}
	eth := layers.Ethernet{
		SrcMAC:       tha,
	...
	if e != nil {
		return []byte{}, errors.New("Could not create ARP reply data:", e.Error())
	}
